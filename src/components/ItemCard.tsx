...
function ItemCard({ item, variant = "default" }: ItemCardProps) {
  ...
  return (
    <article
      className={`rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md dark:border-slate-700 dark:bg-slate-800 ${
        isCompact ? "p-4" : "p-5"
      }`}
    >
      ...
      <h3
        className={`mt-3 font-semibold text-slate-900 dark:text-slate-100 ${
      ...
      <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">📍 {item.location}</p>
